...
Normal startup also recovers a partially running bot: it checks each local service, reuses healthy listeners, and starts only missing services. A service is reused only when its expected application identity is verified — an open port plus HTTP 200 is never enough: the Internal API must answer readiness with `ready=true` and `adapter=postgres`, n8n must answer its health check with the expected `ok` body, and the gateway must additionally pass a signed verification probe proving it validates this machine's channel secret. An occupied port that cannot be positively identified causes the launcher to fail closed: it throws a clear error without killing the occupying process, replacing it, or starting a duplicate. A running but unhealthy service fails its health check without being killed. PostgreSQL must be running for the Internal API readiness check to pass. A running tunnel is reused only after its logged public URL passes the signed webhook check. Opening the launcher again copies the verified current URL without starting duplicate services. Each launcher stops only the processes it created; reused services keep running.
...
## Workflow changes
- Run `node scripts/check-system.mjs --write` after changing the prompt or menu source; this synchronizes the knowledge pack, combined system message, template agent prompt, deterministic router parser, candidate menu-lookup branch, deterministic draft branch, and draft revision.
...
- The `Deterministic FAQ` node in the existing workflow now prepares conversation input: text must set `hasSafeAnswer=false` so it reaches AI Agent and memory. Only unsupported message types receive a fixed fallback. The node emits a deterministic `menuPlan` (price ceiling/exact price, category, name query) but embeds no menu prices; the B2.5 candidate resolves it through the live planner-backed Internal Menu API (`GET /api/v1/menus/mealbox`) into `MENU_CONTEXT` before the AI call. Eligible menu modes skip Gemini entirely via `Build Deterministic Menu Draft` (factual Thai templates, `deterministic-menu` source) and still end at owner approval. Gemini formats remaining facts only and must never invent prices.
