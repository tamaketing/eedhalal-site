(function(){
  ...
  function renderTable(){
    ...
    sortedCats.forEach(function(cat){
      html += '<tr style="background:var(--bg);"><td colspan="5" style="font-weight:900;color:var(--primary);padding:.7rem .9rem;font-size:.85rem">'+cat+' <span style="font-weight:600;color:var(--text-muted)">· '+cats[cat].length+' เมนู</span></td></tr>';
  ...
  function initPlanner(){
    ...
    if(exportAllBtn) exportAllBtn.addEventListener('click', function(){
      ...
      // ship zones js
      var shipJs = '/* auto-export ship zones */\nvar EED_SHIP_ZONES = '+JSON.stringify(getShipZones(), null, 2)+';\nvar EED_SHIP_FREE = '+getFreeThreshold()+';\n';
      setTimeout(function(){ downloadText('shipping-data.js', shipJs, 'application/javascript'); }, 700);
